...
public class EventServicesImpl implements IEventServices{
    ...
    public Event addAffectEvenParticipant(Event event, int idParticipant) {
        ...
        if (participant != null) {
            ...
            // Handle the case where participant is not found, e.g., throw an exception or log an error.
            // You might want to decide on the appropriate behavior for your application.
    ...
    public Event addAffectEvenParticipant(Event event) {
        ...
        for (Participant aParticipant : participants) {
            ...
            if (participant != null) {
                ...
                // Handle the case where participant is not found, e.g., throw an exception or log an error.
                // You might want to decide on the appropriate behavior for your application.
                throw new IllegalArgumentException("Participant not found with ID: " + aParticipant.getIdPart());
            }
        }
        return eventRepository.save(event);
    }
    ...
    public List<Logistics> getLogisticsDates(LocalDate startDate, LocalDate endDate) {
        ...
        List<Logistics> filteredLogisticsList = new ArrayList<>();
            ...
            if (!logisticsSet.isEmpty()) {
                for (Logistics logistics : logisticsSet) {
                    if (logistics.isReserve()) {
                        filteredLogisticsList.add(logistics);
                    }
                }
            }
